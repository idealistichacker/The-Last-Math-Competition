...
原始1个开放PR限制是反刷量的初始默认，不是上游规则。#101仍是简单但合规的已提交解答；#154已完成独立审稿、PDF与最终验证且没有重复投稿。为避免无限等待压住更高质量成果，发布协调器可**仅一次**以 `--max-open-solution-prs 2` 提交#154。

该例外的硬约束：仅限#154；第二个PR后不得再开任何解答PR；仍要在创建前再次刷新题面、全部PR/Issue/文件路径与评论排重；不影响上游维护者审查/合并权限。
