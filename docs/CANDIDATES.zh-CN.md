...
- **#118（P2，fork分支已推送）**：固定二次函数的包、PDF、独立终审和完整协调器验证已完成，fork commit `526ed3cd` 已回读一致；构造仍非常直接，不能靠批量简单投稿替代更有价值贡献。是否开 PR 仍要在当前上游规则、实时排重和显式并行WIP参数下独立决定。
- **#405（高风险研究）**：ordinary-Kostka读法下的固定反例仅为本地原型。2026-09-17审计确认题面关键术语和量词未定义，且“独立审稿通过”无可核验一致证据；在可信澄清或独立 statement-alignment PASS 前不得制作投稿包。
...
`#2617` 不再只依赖手写三元组：本地已在标准 `IncidenceAlgebra ℚ (Fin 2)` 上用实际
`Ideal.jacobson` 得到根非底定理。当前仍只可作为本地研究，等待固定Git依赖状态的独立复审；
不得据此发布第三个解答PR或泛称未固定系数环/半单性含义都已形式化。

### 2026-09-16 #2617 可移植标准Mathlib复审

`#2617` 的新原型已固定 Git 型 Mathlib commit `0df444a...`，在
